...
```bash
git clone https://github.com/yacine-lasri/SuiviStage.git
cd SuiviStage
...
5. **Create an administrator (superuser) account:**
...
* **Application Security:** Gained practical understanding of Web Security basics, including mitigating Cross-Site Request Forgery (CSRF) using Django tokens, handling secure session cookies, and implementing strict backend password hashing.
* **Data Modeling:** Learned how to translate real-world academic workflows into normalized SQL tables using Django's Object-Relational Mapping (ORM), avoiding redundant data.
* **State & Lifecycle Management:** Built logic to handle state transitions (e.g., *Submitted* -> *Under Review* -> *Approved*), which is a fundamental concept in systems and application design.
